...
function generatePins(area: Area): Pin[] {
  ...
  const stations = [
    { name: "North inlet",    sev: "warning"  as Severity, ndci: 0.18, ndti: -0.04 },
    { name: "Central basin",  sev: "critical" as Severity, ndci: 0.21, ndti: -0.06 },
    ...
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content className="fixed inset-0 z-50 bg-background flex flex-col outline-none data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=closed]:animate-out data-[state=closed]:fade-out-0">
          <DialogPrimitive.Title className="sr-only">{area.name}</DialogPrimitive.Title>
          ...
          <header className="border-b px-4 sm:px-6 h-14 flex items-center justify-between gap-3 shrink-0">
            <div className="flex items-center gap-3 min-w-0">
              <div style={{ width: 28, height: 28, borderRadius: 6, background: "var(--accent)", color: "var(--primary)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <MapPin size={15} />
              </div>
              <div className="min-w-0">
                <div className="truncate" style={{ fontSize: 14, fontWeight: 500 }}>{area.name}</div>
                <div className="truncate hidden sm:block" style={{ fontSize: 11, color: "var(--muted-foreground)", fontFamily: "ui-monospace, monospace" }}>
                  W {area.bbox.west} · S {area.bbox.south} · E {area.bbox.east} · N {area.bbox.north}
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 min-h-0 flex flex-col lg:flex-row">
            <div className="relative flex-1 min-h-[320px] lg:min-h-0 bg-secondary">
              ...
              <div className="absolute top-3 left-3 flex flex-col gap-2">
                <div className="bg-card border flex" style={{ borderRadius: 8, padding: 4, gap: 2 }}>
                  <ToggleBtn active={overlay === "ndci"} onClick={() => setOverlay("ndci")}>
                    <Layers size={12} /> NDCI overlay
                  </ToggleBtn>
                  <ToggleBtn active={overlay === "none"} onClick={() => setOverlay("none")}>
                    None
                  </ToggleBtn>
                </div>
              </div>
              <div className="absolute bottom-3 left-3 bg-card border" style={{ borderRadius: 8, padding: "8px 10px", fontSize: 11 }}>
                <div style={{ fontWeight: 500, marginBottom: 4 }}>NDCI legend</div>
                <div className="flex gap-2 items-center" style={{ color: "var(--muted-foreground)" }}>
                  {[
                    { c: "#1f4a72", l: "<0" },
                    { c: "#2c6e7d", l: "0.05" },
                    { c: "#c8b942", l: "0.10" },
              ...
              <div className="absolute bottom-3 right-3 bg-card border" style={{ borderRadius: 8, padding: "6px 10px", fontSize: 11, color: "var(--muted-foreground)" }}>
            ...
            <aside className="lg:w-[360px] lg:border-l border-t lg:border-t-0 overflow-auto bg-card shrink-0">
              <div className="p-5 space-y-5">
                {selectedPin ? (
                  <PinDetail pin={selectedPin} onClear={() => setSelected(null)} />
                ) : (
...
function MapCanvas({
  ...
  return (
    ...
    >
      <defs>
        <pattern id="grid" width={vbW / 12} height={vbH / 12} patternUnits="userSpaceOnUse">
          <path d={`M ${vbW / 12} 0 L 0 0 0 ${vbH / 12}`} fill="none" stroke="var(--border)" strokeWidth={vbW * 0.001} />
        </pattern>
      ...
      <CornerLabel x={west}  y={-north} text={`${west.toFixed(2)}, ${north.toFixed(2)}`}  dx={vbW * 0.005}  dy={-vbH * 0.01}  />
      <CornerLabel x={east}  y={-south} text={`${east.toFixed(2)}, ${south.toFixed(2)}`}  dx={-vbW * 0.005} dy={vbH * 0.025} anchor="end" />
      ...
      {pins.map((p) => {
        ...
        return (
          ...
          >
            <circle cx={projX(p.lon)} cy={projY(p.lat)} r={r * 1.6} fill={sevColor[p.severity]} opacity={0.18} />
            <circle
              cx={projX(p.lon)}
              cy={projY(p.lat)}
              r={r}
...
function AreaSummary({ area, pins, alerts }: { area: Area; pins: Pin[]; alerts: Alert[] }) {
    ...
    <>
      <div>
        ...
        <div style={{ fontSize: 12, color: "var(--muted-foreground)", marginTop: 2 }}>
          {area.active ? "Active monitoring" : "Paused"} · {pins.length} stations · indices {area.indices.join(" · ")}
        </div>
      </div>

      <div className="grid grid-cols-2 gap-2">
        <MetricCard label="Avg NDCI" value={avgNDCI.toFixed(2)} trend="up" trendLabel="across stations" />
        <MetricCard label="Active alerts" value={alerts.filter(a => a.status === "active").length} trend="flat" trendLabel={`${alerts.length} total`} />
        <MetricCard label="Worst station" value={worst.name} trend="up" trendLabel={`NDCI ${worst.ndci.toFixed(2)}`} improving={false} />
      ...
      <div className="border" style={{ borderRadius: 8, padding: 12 }}>
        ...
        <div className="space-y-1.5" style={{ fontSize: 12 }}>
          <StatusRow label="Critical" count={sevCount.critical} color={sevColor.critical} />
          <StatusRow label="Warning"  count={sevCount.warning}  color={sevColor.warning}  />
      ...
      <div>
        ...
        <TrendChart data={trendNDCI} warning={0.10} critical={0.20} />
      </div>

      <div>
        <h3>Recent alerts</h3>
        {alerts.length === 0 ? (
          <div style={{ fontSize: 12, color: "var(--muted-foreground)" }}>No alerts in this area.</div>
        ) : (
          <div className="space-y-2">
            {alerts.slice(0, 4).map((a) => (
              <div key={a.id} className="border flex items-start gap-2" style={{ borderRadius: 8, padding: 10 }}>
                <SeverityBadge severity={a.severity} />
                <div className="flex-1 min-w-0">
                  <div className="truncate" style={{ fontSize: 13, fontWeight: 500 }}>{a.title}</div>
    ...
    <>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div style={{ fontSize: 11, color: "var(--muted-foreground)", fontFamily: "ui-monospace, monospace", marginTop: 2 }}>
            {pin.lat.toFixed(4)}°N, {pin.lon.toFixed(4)}°E
          </div>
        </div>
        <Button variant="ghost" size="sm" onClick={onClear}>Back</Button>
      </div>

      <div className="grid grid-cols-3 gap-2">
        <MetricCard label="NDCI" value={pin.ndci.toFixed(2)} trend={pin.ndci > 0.2 ? "up" : "flat"} trendLabel={pin.ndci > 0.2 ? "above critical" : pin.ndci > 0.1 ? "above warning" : "normal"} improving={pin.ndci <= 0.1} />
        <MetricCard label="NDTI" value={pin.ndti.toFixed(2)} trend="flat" trendLabel="clear" />
        <MetricCard label="NDWI" value={pin.ndwi.toFixed(2)} trend="down" trendLabel="-1% mom" improving={false} />
      </div>

      <div className="border" style={{ borderRadius: 8, padding: 12 }}>
        <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.04em", color: "var(--muted-foreground)", fontWeight: 500, marginBottom: 6 }}>
          Latest reading
        </div>
...
function CornerLabel({ x, y, text, dx, dy, anchor = "start" }: {

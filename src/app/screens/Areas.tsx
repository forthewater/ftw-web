  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-[1100px] mx-auto space-y-6">
      <div className="flex items-start sm:items-center justify-between gap-3">
        ...
        <Button onClick={onAdd}><Plus size={14} /> Add area</Button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {loading
          ? Array.from({ length: 4 }).map((_, i) => <AreaCardSkeleton key={i} />)
          : areas.map((a) => (
          <div key={a.id} className="border bg-card flex flex-col hover:border-[var(--primary)] transition-colors" style={{ borderRadius: 12, padding: 16, gap: 12 }}>
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h3 style={{ marginBottom: 0 }}>{a.name}</h3>
                  {a.activeAlerts > 0 ? (
                    <span style={{ fontSize: 11, fontWeight: 500, padding: "1px 7px", borderRadius: 999, background: "var(--severity-critical-bg)", color: "var(--severity-critical)" }}>
                      {a.activeAlerts} active
                    </span>
                  ) : (
                    <span style={{ fontSize: 11, padding: "1px 7px", borderRadius: 999, background: "var(--secondary)", color: "var(--muted-foreground)" }}>
                      0 active
                    </span>
                  )}
                </div>
                <div style={{ fontSize: 11, color: "var(--muted-foreground)", marginTop: 2, fontFamily: "ui-monospace, monospace" }}>
                  W {a.bbox.west} · S {a.bbox.south} · E {a.bbox.east} · N {a.bbox.north}
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span style={{ fontSize: 11, color: "var(--muted-foreground)" }}>{a.active ? "Active" : "Paused"}</span>
            ...
            <div className="flex items-center justify-between flex-wrap gap-2" style={{ fontSize: 12 }}>
